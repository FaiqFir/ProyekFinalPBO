...
static const char* LIGHT_CARD_HOVER_KATALOG  = "#DBEAFE";
static const char* LIGHT_CARD_HOVER_ADMIN    = "#EDE9FE";
...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent)
{
    setWindowTitle("Sistem Informasi Laptop (SiLap)");
...
void MainWindow::buildMenuAwal()
{
    ...
    // ── Top bar: tombol tema kanan atas ────────────────────────────────────
    ...
    // Logo — gambar PNG
    auto *lblIcon = new QLabel(centerWidget);
    lblIcon->setObjectName("lblIcon");
    lblIcon->setAlignment(Qt::AlignCenter);
    {
        QPixmap pix(":/images/logo-silap.png");
        lblIcon->setPixmap(pix.scaled(120, 120, Qt::KeepAspectRatio, Qt::SmoothTransformation));
    }
    lblIcon->setStyleSheet("background:transparent;");
    vl->addWidget(lblIcon);
    vl->addSpacing(16);

    // Judul — biru-ungu #1E1B4B di light, terang di dark
    auto *lblTitle = new QLabel("SiLap\nSistem Informasi Laptop", centerWidget);
...
void MainWindow::applyTheme()
{
    ...
    const char* themBg  = dark ? DARK_THEME_BG     : LIGHT_THEME_BG;
    const char* themTx  = dark ? DARK_THEME_TXT    : LIGHT_THEME_TXT;
    const char* themBrd = dark ? DARK_THEME_BORDER : LIGHT_THEME_BORDER;
    const char* themHov = dark ? DARK_THEME_HOVER  : LIGHT_THEME_HOVER;

    // Warna judul: biru-ungu di light, terang di dark
    const char* titleColor = dark ? "#E2E8F0" : "#1E1B4B";
    const char* subColor   = dark ? "#94A3B8" : "#475569";
    ...
    if (auto *w = page->findChild<QLabel*>("lblIcon"))
        w->setStyleSheet("background:transparent;");

    if (auto *w = page->findChild<QLabel*>("lblTitle"))
        w->setStyleSheet(QString("font-size:32px; font-weight:bold; color:%1; background:transparent;").arg(titleColor));

    if (auto *w = page->findChild<QLabel*>("lblSub"))
        w->setStyleSheet(QString("font-size:15px; color:%1; background:transparent;").arg(subColor));
    ...
    if (m_btnTheme) {
        // Gunakan teks Unicode yang valid (bukan emoji yang bisa error render)
        QString label = dark ? "☀ Light" : "☾ Dark";
        m_btnTheme->setText(label);
        m_btnTheme->setStyleSheet(
            ...
            .arg(themBg).arg(themBrd).arg(themTx).arg(themHov));
    }

    emit themeChanged(dark);
}

void MainWindow::toggleDarkMode() { m_isDark = !m_isDark; applyTheme(); }

void MainWindow::keHalamanKatalog() { m_stack->setCurrentIndex(1); }

void MainWindow::keHalamanAdmin() {
    LoginDialog loginDlg(m_isDark, this);
    if (loginDlg.exec() == QDialog::Accepted && loginDlg.loginBerhasil())
        m_stack->setCurrentIndex(2);
}

void MainWindow::keMenuAwal() { m_stack->setCurrentIndex(0); }
